...
public class PlayerController : Singleton<PlayerController>
{
    ...
    [SerializeField] private GameObject playerLight = default;
	...
	{
        if (this != null) horizontalMoveInput = ctx.ReadValue<float>();
    }

    private void UpdateVerticalMoveInput(InputAction.CallbackContext ctx)
	{
        if (this != null) verticalMoveInput = ctx.ReadValue<float>();
    }
    ...
    private void Move()
	{
		...
		reachedLadderTop = playerFeetGroundedBuffer > highestLadderEnd - spriteRenderer.bounds.extents.y - groundedRadius && playerFeetGroundedBuffer < highestLadderEnd + spriteRenderer.bounds.extents.y &&
            !boxCollider.IsTouchingLayers(groundLayerMask);

        bool climbingInValidDirection = SetUpLadders(playerFeetGroundedBuffer);               // Variable used to evaluate validity of the direction input when starting to climb, in order to allow it
	...
	private bool SetUpLadders(float pPlayerFeetGroundedBuffer)
	{
        bool climbingInValidDirection = false;
        ...
        if (ladderList.Count > 0 && CurrentCharacterState != CharacterState.Climbing)
		{
            ...
            if (Mathf.Abs(verticalMoveInput) >= moveInputThreshold &&
                ((verticalMoveInput > 0 && pPlayerFeetGroundedBuffer < highestLadderEnd - groundedRadius) || (verticalMoveInput < 0 && pPlayerFeetGroundedBuffer > lowestLadderEnd + groundedRadius)))
            {
                if (!reachedLadderTop)
                {
    ...
    private void ProcessClimbing(float pPlayerFeetGroundedBuffer)
    {
        if (CurrentCharacterState != CharacterState.Climbing)
        {
            ...
            GameManager.Instance.InvokePlayerEnteredClimbingState();
        }
    ...
    {
        if (pCollider.gameObject.layer == LayerMask.NameToLayer(Constants.LayerInteractibleObject))
        {
            interactibleGameObject = pCollider.gameObject;
        }
        else if (pCollider.gameObject.CompareTag(Constants.TagLadder))
        {
            ladderList.Add(pCollider.gameObject);
            UpdateLadderEnds();
        }
        else if (pCollider.gameObject.CompareTag(Constants.TagKey))
		{
            CollectKey(pCollider.gameObject);
		}
        else if (pCollider.gameObject.CompareTag(Constants.TagFunctionalFan))
		{
            inFanArea = true;
            functionalFan = pCollider.gameObject;
		}
        else if (pCollider.gameObject.CompareTag(Constants.TagEnemy))
        {
            if (CurrentCharacterState == CharacterState.Falling && lastFallingStartHeight - pCollider.gameObject.transform.position.y >= bounceMinFallingVerticalDistance)
            {
                pCollider.gameObject.GetComponent<Enemy>().GetStunned();
	...
	private void OnTriggerExit2D(Collider2D pCollider)
    {
        ...
        {
            ladderList.Remove(pCollider.gameObject);
            UpdateLadderEnds();
        }
        else if (pCollider.gameObject.CompareTag(Constants.TagFunctionalFan))
        {
    ...
    private void SetStateToFalling(bool pBouncing)
	{
        if (CurrentCharacterState == CharacterState.Walking)
            AudioManager.Instance.TriggerWwiseEvent(Constants.WwiseEventStopPlayerWalk, gameObject);
    ...
    private void LeaveClimbingState()
	{
        ...
        GameManager.Instance.InvokePlayerExitedClimbingState();
    }
    ...
    private void CollectKey(GameObject pKey)
	{
        numberOfKeys++;
        UIManager.Instance.UpdateKeyNumberText(numberOfKeys);
        pKey.GetComponent<Key>().CollectKey();
    }
